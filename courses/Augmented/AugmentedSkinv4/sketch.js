...
function updateFaceRegionChips() {
  ...
  if (!modelHasFaceRegions()) {
    ui.faceRegionChips.innerHTML = '<span class="small">Face section selection is available for head meshes.</span>';
    return;
  }
...
function applyActiveTextureToSelectedRegions() {
  ...
  if (!modelHasFaceRegions()) {
    if (ui.stats) ui.stats.textContent = "Choose a head mesh before applying SVG face sections.";
    return false;
  }
  if (selectedFaceRegions.size === 0) {
...
const hemiLight = new THREE.HemisphereLight(0xf8fbff, 0x20263a, 1.2);
scene.add(hemiLight);
...
const fillLight = new THREE.DirectionalLight(0xbfd7ff, 0.75);
fillLight.position.set(-240, 80, 180);
scene.add(fillLight);

const rimLight = new THREE.DirectionalLight(0xffffff, 0.55);
rimLight.position.set(0, -140, -260);
scene.add(rimLight);
...
function fixTriangleWinding(mesh) {
  if (!mesh || !mesh.positions || !mesh.triangles) return;
...
function computeSmoothVertexNormals(mesh) {
  const positionalNormals = new Map();
  const positionKey = (p) => `${p.x.toFixed(4)},${p.y.toFixed(4)},${p.z.toFixed(4)}`;
  ...
  for (const tri of mesh.triangles) {
    ...
    const weightedNormal = new THREE.Vector3().crossVectors(
      new THREE.Vector3().subVectors(b, a),
      new THREE.Vector3().subVectors(c, a)
    );

    if (weightedNormal.lengthSq() < 1e-12) continue;

    for (let k = 0; k < 3; k++) {
      const p = mesh.positions[tri[k]];
      const key = positionKey(p);
      if (!positionalNormals.has(key)) positionalNormals.set(key, new THREE.Vector3());
      positionalNormals.get(key).add(weightedNormal);
    }
  }

  mesh.vertexNormals = mesh.positions.map((p) => {
    const n = positionalNormals.get(positionKey(p));
    return n && n.lengthSq() > 1e-12 ? n.clone().normalize() : p.clone().normalize();
  });
}
...
function buildBaseSurface(mesh) {
  ...
  const flatPositions = [];
  const flatNormals = [];
  mesh.positions.forEach((pnt, idx) => {
    flatPositions.push(pnt.x, pnt.y, pnt.z);
    const n = mesh.vertexNormals && mesh.vertexNormals[idx]
      ? mesh.vertexNormals[idx]
      : pnt.clone().normalize();
    flatNormals.push(n.x, n.y, n.z);
  });
  geometry.setAttribute("position", new THREE.Float32BufferAttribute(flatPositions, 3));
