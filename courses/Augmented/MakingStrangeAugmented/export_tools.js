...
(function () {
  ...
  function buildProjectManifest(options = {}) {
    ...
    return {
      ...
      meshSource: {
        type: "parametric-preset-json",
        modelMode: ctx.meshData ? ctx.meshData.modelMode : "unknown",
        modelLabel: ctx.meshData ? ctx.meshData.modelLabel : "unknown",
        vertexCount: ctx.meshData ? ctx.meshData.positions.length : 0,
        triangleCount: ctx.meshData ? ctx.meshData.triangles.length : 0
      },

      faceShapeSettings: {
        baseDepthScale: Number(ctx.ui.modelDepthScale?.value || 0),
        faceRoundness: Number(ctx.ui.morphFaceRoundness?.value || 0),
        jawWidth: Number(ctx.ui.morphJawWidth?.value || 0),
        chinLength: Number(ctx.ui.morphChinLength?.value || 0),
        cheekbones: Number(ctx.ui.morphCheekbones?.value || 0),
        noseWidth: Number(ctx.ui.morphNoseWidth?.value || 0),
        noseProjection: Number(ctx.ui.morphNoseProjection?.value || 0),
        eyeSpacing: Number(ctx.ui.morphEyeSpacing?.value || 0),
        eyeSize: Number(ctx.ui.morphEyeSize?.value || 0),
        foreheadHeight: Number(ctx.ui.morphForeheadHeight?.value || 0),
        ageImpression: Number(ctx.ui.morphAge?.value || 0),
        angularSoftPresentation: Number(ctx.ui.morphPresentation?.value || 0)
      },
      ...
      printSafetySettings: {
        limitNonplanarOffset: !!ctx.ui.limitNonplanarOffset?.checked,
        maxNonplanarOffset: Number(ctx.ui.maxNonplanarOffset?.value || 0),
        protectFeatureRegions: !!ctx.ui.protectFeatureRegions?.checked,
        featureProtectRadius: Number(ctx.ui.featureProtectRadius?.value || 0),
        featureProtectFalloff: Number(ctx.ui.featureProtectFalloff?.value || 0)
      },
      ...
      exportSettings: {
        ...
        colorColumns: ["r", "g", "b", "a", "color_hex"]
      },
    ...
    return await new Promise((resolve) => {
      function processChunk() {
        ...
        while (
          ...
          const rules = ctx.applyDisplacementRules
            ? ctx.applyDisplacementRules(rawDisplacement, pAndN.p.x, pAndN.p.y, pAndN.p.z)
            : { displacement: rawDisplacement };
          const finalPos = pAndN.p.clone().addScaledVector(pAndN.n, baseOffset + rules.displacement);

          const colorHex = "#" + [sample.r, sample.g, sample.b]
  ...
  async function buildRenderedPointsCSVWorker() {
    ...
    // Copy so transferring buffers to the Worker does not detach live geometry.
    const positions = new Float32Array(posAttr.array);
    const colors = colAttr ? new Float32Array(colAttr.array) : null;
    const total = posAttr.count;

    /*
      Build the worker script as an array of lines.

      Important: do not put rows.join("\n") inside a template literal. Some
      browser/editor combinations turn that into a literal line break inside
      a quoted string in the Worker blob, causing:

        SyntaxError: Invalid or unexpected token

      The Worker uses String.fromCharCode(10) instead.
    */
    const workerLines = [
      ...
      "  rows[0] = 'point_index,stage,x,y,z,r,g,b,a,color_hex';",
      ...
      "    const hex = '#' + [r, g, b].map(function(v) {",
      ...
      "    ].join(',');",
      "  }",
      "  self.postMessage(rows.join(NL));",
      "};"
    ];

    const workerCode = workerLines.join(String.fromCharCode(10));
  ...
  async function exportViewImages() {
    const images = await generateViewImages(false);
    const slug = FSUtils.timestampSlug();

    for (const img of images) {
      FSUtils.downloadBlob(
        FSUtils.dataURLToBlob(img.dataURL),
        `floating_skin_${img.name}_${slug}.png`
      );
      await FSUtils.nextFrame();
    }
  }
  ...
  async function exportFullProjectZip() {
    ...
    status("Project ZIP complete. View images are exported separately, not stored inside this ZIP.");
  }

  function attachExportButtonHandlers() {
    const ids = {
      previewViewsBtn: () => generateViewImages(true),
      exportViewsBtn: exportViewImages,
  ...
  window.FSExportTools = {
    ...
    exportManifestJSON,
    exportViewImages,
    exportViewsZip: exportViewImages,
