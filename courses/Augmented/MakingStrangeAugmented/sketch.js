...
if (typeof THREE.OrbitControls === "undefined") {
  throw new Error(
    "THREE.OrbitControls is not loaded. Check the OrbitControls script tag in index.html."
  );
}
...
const MODEL_SOURCES = {
  ...
  mediapipe: "./mediapipe_face_mesh.json",
...
const MODEL_FLAGS = {
  face: {
    isHead: true,
    allowManualMorphs: true,
    applyDepthScale: true,
    defaultFlipY: true,
  },
  mediapipe: {
    isHead: true,
    allowManualMorphs: false,
    applyDepthScale: false,
    defaultFlipY: true,
  },
  student_bald3: {
    isHead: true,
...
function modelFlag(mode, key, fallback = false) {
  return MODEL_FLAGS[mode] && key in MODEL_FLAGS[mode]
    ? MODEL_FLAGS[mode][key]
    : fallback;
}
...
const FACE_POINTSETS =
  FACE_REGION_DATA && FACE_REGION_DATA.pointsets
    ? FACE_REGION_DATA.pointsets
    : {};
...
const FACE_REGION_REFERENCE_TRIANGLES = (() => {
  if (!FACE_REGION_DATA || !FACE_REF_UVS.length || !FACE_REF_TRIS.length)
    return [];
...
const ui = {
  ...
  advancedHeadDepthGroup: document.getElementById("advancedHeadDepthGroup"),
  advancedBrushGroup: document.getElementById("advancedBrushGroup"),
  ...
  limitToSelectedFaceRegions: document.getElementById(
    "limitToSelectedFaceRegions"
  ),
  ...
  applySelectedTextureToRegionsBtn: document.getElementById(
    "applySelectedTextureToRegionsBtn"
  ),
  ...
  useCylindricalProjectionTexture: document.getElementById(
    "useCylindricalProjectionTexture"
  ),
  useFrontProjectionTexture: document.getElementById(
    "useFrontProjectionTexture"
  ),
  ...
  featureProtectRadiusValue: document.getElementById(
    "featureProtectRadiusValue"
  ),
  featureProtectFalloff: document.getElementById("featureProtectFalloff"),
  featureProtectFalloffValue: document.getElementById(
    "featureProtectFalloffValue"
  ),
...
function resetFaceMorphs() {
  ...
  switchModel(currentModelMode).catch((err) => {
    console.error(err);
    if (ui.stats)
      ui.stats.textContent = "Could not reset face shape: " + err.message;
  });
}
...
function applyManualFaceMorphs(positions, uvs) {
  ...
  for (const p of positions) {
    ...
    const front = smoothWeight01(zn, -0.3, 0.9);
    const lower = smoothWeight01(-yn, 0.12, 0.9);
    const upper = smoothWeight01(yn, 0.18, 0.9);
    ...
    const chinMask =
      gaussianWeight(yn, -0.82, 0.18) * gaussianWeight(xn, 0, 0.55);
    const cheekMask =
      gaussianWeight(yn, 0.02, 0.22) * smoothWeight01(ax, 0.2, 0.72) * front;
    const foreheadMask = upper * gaussianWeight(xn, 0, 0.86);
    const noseMask = centerBand * gaussianWeight(yn, -0.03, 0.3) * front;
    const leftEyeMask =
      gaussianWeight(xn, -0.32, 0.18) * gaussianWeight(yn, 0.28, 0.13) * front;
    const rightEyeMask =
      gaussianWeight(xn, 0.32, 0.18) * gaussianWeight(yn, 0.28, 0.13) * front;
    ...
    p.x +=
      leftEyeMask * (-4.5 * eyeSpacing) + rightEyeMask * (4.5 * eyeSpacing);
...
function syncUIText() {
  if (ui.modelDepthScaleValue && ui.modelDepthScale) {
    ui.modelDepthScaleValue.textContent = Number(
      ui.modelDepthScale.value || 1.0
    ).toFixed(2);
  }
  syncMorphUIText();
...
[
  ...
  .filter(Boolean)
  .forEach((el) => el.addEventListener("input", syncUIText));
...
function forceNeutralShapeStageValues() {
  ...
  if (ui.modelDepthScale) ui.modelDepthScale.value = "0.90";
  syncUIText();
}
...
function applyDeviceLayout(options = {}) {
  ...
  if (!deviceDefaultsApplied || options.forceDefaults) {
    if (profile === "phone") {
      if (ui.maxFloatingPoints)
        ui.maxFloatingPoints.value = Math.min(
          Number(ui.maxFloatingPoints.value || 90000),
          90000
        );
      if (ui.sampleStep)
        ui.sampleStep.value = Math.max(Number(ui.sampleStep.value || 2), 2);
      if (ui.pointSize) ui.pointSize.value = Number(ui.pointSize.value || 1.5);
    } else if (profile === "tablet") {
      if (ui.maxFloatingPoints)
        ui.maxFloatingPoints.value = Math.min(
          Number(ui.maxFloatingPoints.value || 180000),
          180000
        );
      if (ui.sampleStep)
        ui.sampleStep.value = Math.max(Number(ui.sampleStep.value || 1), 1);
    }
  ...
  if (ui.deviceStatus) {
    ...
    ui.deviceStatus.textContent = `${
      requested === "auto" ? "Auto" : "Manual"
    }: ${profile} layout · ${window.innerWidth}×${
      window.innerHeight
    } · ${coarseText}`;
  }
...
function uvToDesignPixel(u, v) {
  if (
    meshData &&
    meshData.uvBounds &&
    ui.fitTextureToFaceUvBounds &&
    ui.fitTextureToFaceUvBounds.checked
  ) {
    ...
    return {
      x: fu * (DESIGN_W - 1),
      y:
        ui.flipDesignY && ui.flipDesignY.checked
          ? fv * (DESIGN_H - 1)
          : (1 - fv) * (DESIGN_H - 1),
    };
  }
...
function designPixelForBarycentric(tri, a, b, c) {
  ...
  if (
    ui.useCylindricalProjectionTexture &&
    ui.useCylindricalProjectionTexture.checked
  ) {
...
function triangleRegionName(mesh, triIndex) {
  ...
  if (mesh.triangleRegionNames && mesh.triangleRegionNames[triIndex])
    return mesh.triangleRegionNames[triIndex];
...
function shouldUseTriangleForSelectedRegions(tri) {
  ...
  if (!tri || !tri.regionName) return false;
...
function updateFaceRegionChips() {
  ...
  if (!modelHasFaceRegions()) {
    ui.faceRegionChips.innerHTML =
      '<span class="small">Face section selection is available for head meshes.</span>';
    return;
  }
  if (selectedFaceRegions.size === 0) {
    if (faceRegionAssignments.size === 0) {
      ui.faceRegionChips.innerHTML =
        '<span class="small">No active target. Click SVG sections, then choose a source and apply it.</span>';
      return;
    }
    ...
    [...faceRegionAssignments.entries()]
      .sort()
      .forEach(([id, textureIndex]) => {
        const chip = document.createElement("span");
        chip.className = "regionChip assignedChip";
        const tex = textureChoices[textureIndex];
        chip.textContent = `${faceRegionLabel(id)} ← ${
          tex ? tex.label : "source"
        }`;
  ...
  [...selectedFaceRegions].sort().forEach((id) => {
    ...
    const tex = faceRegionAssignments.has(id)
      ? textureChoices[faceRegionAssignments.get(id)]
      : null;
    chip.textContent = tex
      ? `${faceRegionLabel(id)} ← ${tex.label}`
      : faceRegionLabel(id);
    ui.faceRegionChips.appendChild(chip);
  });
}
...
function buildSelectedRegionMaskInfo(regions) {
  ...
  ctx.fillStyle = "white";
  const tris =
    meshData.triangleData && meshData.triangleData.length
  ...
  const pad = Math.max(
    ...
    Math.min(
      28,
      Math.max(bounds.maxX - bounds.minX, bounds.maxY - bounds.minY) * 0.04
    )
  );
...
function drawTextureChoiceIntoMask(textureIndex, regions) {
  ...
  let drewAny = false;
  const regionList = [...regions].filter((region) => region);
...
function redrawDesignFromLayerState(options = {}) {
  ...
  if (fullFaceTextureIndex >= 0 && textureChoices[fullFaceTextureIndex]) {
    drawImageCover(
      dctx,
      textureChoices[fullFaceTextureIndex].source,
      0,
      0,
      DESIGN_W,
      DESIGN_H
  ...
  if (
    ...
    typeof rebuildFloatingGeometry === "function" &&
    meshData
  ) {
...
function applyActiveTextureToSelectedRegions() {
  if (activeTextureIndex < 0 || !textureChoices[activeTextureIndex])
    return false;
  if (!modelHasFaceRegions()) {
    if (ui.stats)
      ui.stats.textContent =
        "Choose a head mesh before applying SVG face sections.";
    return false;
  }
  if (selectedFaceRegions.size === 0) {
    if (ui.stats)
      ui.stats.textContent =
        "Choose one or more SVG face sections first, then click ‘Apply source to selected’. Use the full-face button for a whole-face background.";
    return false;
  }

  // Region painting is UV-space, so switch to fitted UV mapping to keep the SVG
  // sections aligned with the rendered face.
  if (ui.useCylindricalProjectionTexture)
    ui.useCylindricalProjectionTexture.checked = false;
  if (ui.useFrontProjectionTexture)
    ui.useFrontProjectionTexture.checked = false;
...
const blurredDctx = blurredDesignCanvas.getContext("2d", {
...
function updateBlurredDesign() {
  ...
  if (blurPreviewCtx && blurPreviewCanvas) {
    blurPreviewCtx.clearRect(
      0,
      0,
      blurPreviewCanvas.width,
      blurPreviewCanvas.height
    );
    blurPreviewCtx.drawImage(
      blurredDesignCanvas,
      0,
      0,
      blurPreviewCanvas.width,
...
function getActiveDesignContext() {
  return ui.useBlurredDesign && ui.useBlurredDesign.checked
    ? blurredDctx
    : dctx;
}
...
function addBuiltInTextures() {
  makeTextureCanvas("Circles", drawGeometricCirclesPattern, {
    fillMode: "cover",
  });
...
function makeUploadedCompositeCanvas(items) {
  ...
  for (const item of items) {
    ...
    drawImageCover(
      ctx,
      item.source || item.canvas || item.img,
      0,
      0,
      c.width,
      c.height
...
function addCapturedFaceTextureIfAvailable() {
  let dataURL = null;
  try {
    dataURL = localStorage.getItem("floatingSkinCapturedFaceTexturePNG");
  } catch (e) {}
  if (!dataURL) return;
  ...
  img.onload = () => {
    const savedAt = (() => {
      try {
        return localStorage.getItem("floatingSkinCapturedFaceSavedAt") || "";
      } catch (e) {
        return "";
      }
    })();
    ...
    const idx = addTextureChoice(fullCanvasSource, label, {
      fillMode: "cover",
    });
    if (ui.stats) {
      ui.stats.textContent =
        "Captured face photo is available as a texture source. Select it, then apply it to the full face or selected SVG regions.";
    }
  };
  img.onerror = () => {
    if (ui.stats)
      ui.stats.textContent =
        "Captured face texture was saved but could not be loaded. Recapture the face.";
  };
  img.src = dataURL;
}
...
function profileReferenceSummary() {
  ...
  return views.map((name) => {
    ...
    try {
      ...
      savedAt =
        localStorage.getItem(`floatingSkinProfile_${name}_savedAt`) || "";
    } catch (e) {}
...
function updateProfileReferenceStatus() {
  ...
  const parts = profileReferenceSummary().map(
    (r) => `${r.name}: ${r.has ? "saved" : "missing"}`
  );
  ui.profileReferenceStatus.textContent =
    parts.join(" | ") +
    ". These are reference photos for the upcoming side-profile reconstruction step.";
}
...
function loadTextureFiles(files) {
  ...
  const finishOne = () => {
    ...
    if (loaded.length > 1) {
      ...
      const compositeIndex = addTextureChoice(
        composite,
        `Uploaded composite (${loaded.length})`,
        {
          fillMode: "cover",
        }
      );
      ...
      if (ui.stats)
        ui.stats.textContent =
          "Uploaded composite selected. Click SVG sections and apply to selected regions, or apply to full face.";
    } else if (lastAddedIndex >= 0) {
      activeTextureIndex = lastAddedIndex;
      updateTextureSelectionUI();
      if (ui.stats)
        ui.stats.textContent =
          "Uploaded texture selected. Click SVG sections and apply to selected regions, or apply to full face.";
    }
  };
  ...
  for (const file of fileList) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        lastAddedIndex = addTextureChoice(fullCanvasSource, file.name, {
          fillMode: "cover",
        });
        finishOne();
      };
...
// Keep the original lighting available for the MediaPipe face. Imported and
// procedural full heads use a softer three-light setup so their sculpted volume
// reads clearly without changing the MediaPipe checkpoint.
const hemiLight = new THREE.HemisphereLight(0xffffff, 0x223344, 1.1);
scene.add(hemiLight);
...
const fillLight = new THREE.DirectionalLight(0xc8dcff, 0.0);
fillLight.position.set(-2.5, 1.0, 2.0);
scene.add(fillLight);

const rimLight = new THREE.DirectionalLight(0xffffff, 0.0);
rimLight.position.set(0.5, -1.5, -2.5);
scene.add(rimLight);

function applyLightingForModel(mode) {
  if (mode === "mediapipe") {
    // Preserve the known-good MediaPipe appearance exactly.
  ...
  // Softer, more even studio-style lighting for closed/full head meshes.
...
async function loadMesh(url) {
  ...
  if (String(url).startsWith("localStorage:")) {
    ...
    if (!saved) {
      throw new Error(
        "No captured face is saved yet. Open Live capture, freeze a face, then choose ‘Use this capture in Designer.’"
      );
    }
...
function meshFromRaw(raw, sourceLabel = "raw mesh") {
  const positions = raw.positions.map(
  ...
  const allowDepthScale =
    raw.applyDepthScale != null
      ? !!raw.applyDepthScale
      : currentModelFlag("applyDepthScale", false);
  const depthScale =
    allowDepthScale && ui.modelDepthScale
      ? parseFloat(ui.modelDepthScale.value || "1.00")
      : 1.0;
  const faceVertexCount = positions.length;
  if (allowDepthScale) {
...
function computeLegacyVertexNormals(mesh) {
...
function computeSmoothedHeadNormals(mesh) {
  // Imported GLB heads commonly duplicate vertices along UV/material seams.
  // Averaging only by vertex index leaves visible lighting patches at those
  // seams. Group coincident positions, orient each face contribution outward,
  // and average the area-weighted normals without changing triangle topology.
  const bounds = new THREE.Box3().setFromPoints(mesh.positions);
  const center = bounds.getCenter(new THREE.Vector3());
  const size = bounds.getSize(new THREE.Vector3());
  const tolerance = Math.max(size.x, size.y, size.z, 1) * 1e-5;
  const invTolerance = 1 / tolerance;
  const keyFor = (p) =>
    `${Math.round(p.x * invTolerance)},${Math.round(
      p.y * invTolerance
    )},${Math.round(p.z * invTolerance)}`;
  ...
  for (const tri of mesh.triangles) {
    ...
    const faceCenter = new THREE.Vector3()
      ...
      .add(c)
      .multiplyScalar(1 / 3);
    const outward = faceCenter.sub(center);
    if (faceNormal.dot(outward) < 0) faceNormal.negate();

    for (let corner = 0; corner < 3; corner++) {
      const key = keyFor(mesh.positions[tri[corner]]);
  ...
  mesh.vertexNormals = mesh.positions.map((p) => {
    const sum = sums.get(keyFor(p));
    if (sum && sum.lengthSq() > 1e-16) return sum.clone().normalize();
    const fallback = p.clone().sub(center);
    return fallback.lengthSq() > 1e-16
      ? fallback.normalize()
...
function computeVertexNormals(mesh) {
  if (currentModelMode === "mediapipe") {
    computeLegacyVertexNormals(mesh);
  } else {
    computeSmoothedHeadNormals(mesh);
  }
}
...
function buildTriangleData(mesh) {
  ...
  mesh.uvCoverage = Math.max(
    0.01,
    Math.min(1, totalUvAreaPx / (DESIGN_W * DESIGN_H))
  );
  mesh.sampleFilter = {
    maxEdge99: edge99,
...
function buildBaseWire(mesh) {
  ...
  baseWire.visible =
    currentWorkflowStage === "shape" ? true : ui.showMesh.checked;
  contentGroup.add(baseWire);
}

function buildBaseSurface(mesh) {
  if (baseSurface) {
    contentGroup.remove(baseSurface);
    if (baseSurface.geometry) baseSurface.geometry.dispose();
    if (baseSurface.material) baseSurface.material.dispose();
  }
  ...
  const isMediaPipe = currentModelMode === "mediapipe";

  if (isMediaPipe) {
    // Do not alter the known-good MediaPipe normal path.
    geometry.computeVertexNormals();
  } else {
    const flatNormals = [];
    mesh.vertexNormals.forEach((n) => flatNormals.push(n.x, n.y, n.z));
    geometry.setAttribute(
      "normal",
      new THREE.Float32BufferAttribute(flatNormals, 3)
    );
    geometry.attributes.normal.needsUpdate = true;
  }

  const materialOptions = isMediaPipe
    ? {
        color: 0xd8d0c8,
        roughness: 0.72,
        metalness: 0.02,
...
function findContainingTriangleAtUvPixel(mesh, accel, x, y) {
  const cx = Math.max(
    0,
    Math.min(accel.cols - 1, Math.floor(x / accel.cellSize))
  );
  const cy = Math.max(
    0,
    Math.min(accel.rows - 1, Math.floor(y / accel.cellSize))
  );
  const eps = 1e-5;
  ...
  for (let oy = -1; oy <= 1; oy++) {
    for (let ox = -1; ox <= 1; ox++) {
      const ncx = cx + ox;
      const ncy = cy + oy;
      if (ncx < 0 || ncx >= accel.cols || ncy < 0 || ncy >= accel.rows)
        continue;
  ...
  for (const triIndex of candidates) {
    ...
    if (
      ui.filterStretchedTriangles &&
      ui.filterStretchedTriangles.checked &&
      !tri.sampleable
    ) {
      continue;
    }
...
function buildSurfaceSamplePlan(mesh, targetCount) {
  ...
  const useFilter =
    ui.filterStretchedTriangles && ui.filterStretchedTriangles.checked;
...
function sampleTriangleBarycentric(sampleIndex, sampleCount, triIndex) {
  ...
  let u = fract(
    (sampleIndex + 0.5) * 0.7548776662466927 + (triIndex + 1) * 0.137
  );
  let v = fract(
    (sampleIndex + 0.5) * 0.5698402909980532 + (triIndex + 1) * 0.371
  );
...
function buildUvLookup(mesh, sampleStep) {
  ...
  */
  ...
  const safeMaxPoints =
    Number.isFinite(maxPoints) && maxPoints > 0 ? maxPoints : 360000;
...
function protectionRamp(d, radius, falloff, minimumWeight) {
  ...
  return (
    minimumWeight +
    (1 - minimumWeight) * smoothstep01((d - radius) / Math.max(1e-9, falloff))
  );
}
...
function featureProtectionWeight(x, y, z) {
  ...
  const eyeMouthD = nearestLandmarkDistance(
    EYE_MOUTH_PROTECTED_LANDMARKS,
    x,
    y,
    z
  );
  ...
  const noseD = nearestLandmarkDistance(
    NOSE_SHAPE_PROTECTED_LANDMARKS,
    x,
    y,
    z
  );
...
function buildFloatingPoints(mesh, uvLookupObj) {
  ...
  */
  ...
  const targetCount =
    uvLookupObj.targetCount || uvLookupObj.maxPoints || 360000;
  const allocations =
    uvLookupObj.allocations ||
    buildSurfaceSamplePlan(mesh, targetCount).allocations;
  ...
  function processBatch() {
    ...
    while (
      ...
      const tri = mesh.triangleData[triIndex];
      const bary = sampleTriangleBarycentric(
        localIndex,
        triSampleCount,
        triIndex
      );
      ...
      const invLen =
        1 / Math.max(1e-12, Math.sqrt(nx * nx + ny * ny + nz * nz));
      nx *= invLen;
      ny *= invLen;
      nz *= invLen;
    ...
    geometry.setAttribute(
      ...
      new THREE.Float32BufferAttribute(
        positions.subarray(0, renderedCount * 3),
        3
      )
    );
    geometry.setAttribute(
      "color",
      new THREE.Float32BufferAttribute(colors.subarray(0, renderedCount * 3), 3)
    );
...
function buildTriangleCenterGlyphs(mesh) {
  ...
  for (const tri of mesh.triangleData) {
    ...
    const rules = applyDisplacementRules(
      rawDelta,
      tri.center.x,
      tri.center.y,
      tri.center.z
...
base face depth scale: ${
    ui.modelDepthScale
      ? Number(ui.modelDepthScale.value || 0).toFixed(2)
      : "n/a"
  }
manual face sliders active: ${
    getMorphValues
      ? Object.values(getMorphValues()).some((v) => Math.abs(v) > 1e-6)
        ? "yes"
        : "no"
      : "n/a"
  }
face SVG regions: ${
    modelHasFaceRegions()
      ? selectedFaceRegions.size
        ? [...selectedFaceRegions].map(faceRegionLabel).join(", ")
        : "none selected"
...
print-safe clamp:
${ui.limitNonplanarOffset && ui.limitNonplanarOffset.checked ? "on" : "off"} ±${
    ui.maxNonplanarOffset ? ui.maxNonplanarOffset.value : "n/a"
  }
...
function showFloatingPointsOnly() {
  if (ui.showMesh) ui.showMesh.checked = false;
  if (ui.showPoints) ui.showPoints.checked = true;
  if (ui.showTriGlyphs) ui.showTriGlyphs.checked = false;

  if (baseSurface) baseSurface.visible = false;
  if (baseWire) baseWire.visible = false;
  if (triGlyphs) triGlyphs.visible = false;
  if (floatingPoints) floatingPoints.visible = true;
}
...
function rebuildFloatingGeometry() {
  ...
  if (triGlyphs)
    triGlyphs.visible =
      currentWorkflowStage === "texture" && ui.showTriGlyphs.checked;
...
function stageGroupList() {
  return {
    shape: [ui.baseHeadGroup, ui.advancedHeadDepthGroup, ui.faceMorphGroup],
    texture: [
...
function setWorkflowStage(stage, options = {}) {
  ...
  document.body.classList.toggle(
    "stage-shape",
    currentWorkflowStage === "shape"
  );
  document.body.classList.toggle(
    "stage-texture",
    currentWorkflowStage === "texture"
  );

  const groups = stageGroupList();
  groups.shape.forEach((g) =>
    setGroupVisible(g, currentWorkflowStage === "shape")
  );
  groups.texture.forEach((g) =>
    setGroupVisible(g, currentWorkflowStage === "texture")
  );
  groups.optional.forEach((g) => setGroupVisible(g, false));

  if (ui.shapeStageBtn)
    ui.shapeStageBtn.classList.toggle(
      "activeStage",
      currentWorkflowStage === "shape"
    );
  if (ui.textureStageBtn)
    ui.textureStageBtn.classList.toggle(
      "activeStage",
      currentWorkflowStage === "texture"
    );
  if (ui.continueToTextureBtn)
    ui.continueToTextureBtn.style.display =
      currentWorkflowStage === "shape" ? "inline-block" : "none";
  if (ui.continueToTextureBtn2)
    ui.continueToTextureBtn2.style.display =
      currentWorkflowStage === "shape" ? "inline-block" : "none";
  if (ui.backToShapeBtn)
    ui.backToShapeBtn.style.display =
      currentWorkflowStage === "texture" ? "inline-block" : "none";

  if (ui.workflowStatus) {
    ui.workflowStatus.textContent =
      currentWorkflowStage === "shape"
        ? "State 1: choose a starting head and rotate it. Advanced shape/depth controls are available at the bottom, but images, textures, brushes, point clouds, and exports stay hidden."
        : "State 2: the shaped head is locked in as the base. Now choose built-in patterns or upload images and assign them to SVG face regions. Advanced brush tools are available at the bottom.";
  }
  ...
  const shapeStage = currentWorkflowStage === "shape";

  if (shapeStage) {
    if (baseSurface)
      baseWire.visible =
        currentWorkflowStage === "shape" &&
        (!ui.showMesh || ui.showMesh.checked);

    if (baseWire) {
      baseWire.visible = !ui.showMesh || ui.showMesh.checked;
    }
...
Next step:
click “Continue to image / texture stage” when the head choice/shape is ready.`;
}
...
function scheduleFaceMorphUpdate() {
  ...
  morphUpdateTimer = setTimeout(() => {
    switchModel("face").catch((err) => {
      console.error(err);
      if (ui.stats)
        ui.stats.textContent = "Error updating face shape: " + err.message;
    });
  }, 45);
}
...
// ------------------------------------------------------------
if (ui.shapeStageBtn)
  ui.shapeStageBtn.addEventListener("click", () => setWorkflowStage("shape"));
if (ui.textureStageBtn)
  ui.textureStageBtn.addEventListener("click", () =>
    setWorkflowStage("texture")
  );
if (ui.continueToTextureBtn)
  ui.continueToTextureBtn.addEventListener("click", () =>
    setWorkflowStage("texture")
  );
if (ui.continueToTextureBtn2)
  ui.continueToTextureBtn2.addEventListener("click", () =>
    setWorkflowStage("texture")
  );
if (ui.backToShapeBtn)
  ui.backToShapeBtn.addEventListener("click", () => setWorkflowStage("shape"));
...
ui.showPoints.addEventListener("change", () => {
  if (floatingPoints) {
    floatingPoints.visible =
      currentWorkflowStage === "texture" &&
      ui.showPoints.checked;
  }
});

ui.showTriGlyphs.addEventListener("change", () => {
  ui.showTriGlyphs.checked = false;

  if (triGlyphs) {
    triGlyphs.visible = false;
  }
});
if (ui.flipY) {
...
[
  ...
  .filter(Boolean)
  .forEach((el) => el.addEventListener("change", rebuildFloatingGeometry));
...
if (ui.modelMode) {
  ui.modelMode.addEventListener("change", () => {
    switchModel(ui.modelMode.value).catch((err) => {
      console.error(err);
      if (ui.stats)
        ui.stats.textContent = "Error switching model: " + err.message;
    });
  });
}
...
if (ui.resetMorphsBtn)
  ui.resetMorphsBtn.addEventListener("click", resetFaceMorphs);
...
if (ui.selectAllFaceRegionsBtn)
  ui.selectAllFaceRegionsBtn.addEventListener("click", selectAllFaceRegions);
if (ui.clearFaceRegionsBtn)
  ui.clearFaceRegionsBtn.addEventListener("click", clearFaceRegions);
if (ui.selectCoreFaceRegionsBtn)
  ui.selectCoreFaceRegionsBtn.addEventListener("click", selectCoreFaceRegions);
if (ui.applySelectedTextureToRegionsBtn)
  ui.applySelectedTextureToRegionsBtn.addEventListener(
    "click",
    applyActiveTextureToSelectedRegions
  );
if (ui.limitToSelectedFaceRegions)
  ui.limitToSelectedFaceRegions.addEventListener(
    "change",
    rebuildFloatingGeometry
  );
...
function configureMappingForModel(mode) {
  ...
  if (studentHeadMode) {
    // The MediaPipe face and imported student heads already carry aligned UVs.
    // Use those UVs directly instead of re-projecting from x/y/z.
    if (ui.useCylindricalProjectionTexture)
      ui.useCylindricalProjectionTexture.checked = false;
    if (ui.useFrontProjectionTexture)
      ui.useFrontProjectionTexture.checked = false;
    ...
    if (ui.useCylindricalProjectionTexture)
      ui.useCylindricalProjectionTexture.checked = true;
    if (ui.useFrontProjectionTexture)
      ui.useFrontProjectionTexture.checked = true;
    if (ui.fitTextureToFaceUvBounds) ui.fitTextureToFaceUvBounds.checked = true;
  }
}
...
async function init() {
  ...
  const requestedStage =
    params.get("stage") === "texture" ? "texture" : "shape";
  setWorkflowStage(requestedStage, { skipRebuild: true });
  const initialMode =
    ui.modelMode && MODEL_SOURCES[ui.modelMode.value]
      ? ui.modelMode.value
      : "face";
